...
describe Game do
  ...
  before do
    ...
    test_game.setup(test_player_1,test_player_2, player_class)
  end

  describe '#attack' do
    it 'attacks a player and reduces their hit points by 10' do
      allow(test_player_2).to receive(:attack)
      allow(test_player_1).to receive(:attack)
  ...
  describe '#new' do
    it 'accepts two players' do
      expect(Game.instance).to respond_to(:setup).with(2).arguments
    end
  end
